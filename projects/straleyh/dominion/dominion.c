...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  if (nextPlayer > (state->numPlayers - 1)){
	...
	if(card == adventurer) {
		adventurerEffect(drawntreasure, state, currentPlayer);
	}
	else if(card == smithy) {
		...
		remodelEffect(currentPlayer, state, handPos, choice1, choice2);
	}
  else {
  //uses switch to select card and perform actions
  switch( card ) 
    {
    //case adventurer: //Moved to own if statement
    ...
    //case smithy://Moved to own else if statement
